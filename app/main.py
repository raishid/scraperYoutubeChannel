...
def api_linkedin():
    ...
    rs = requests.get(
        f'{os.environ.get("API_URL")}/socials/accounts/linkedin',
        headers={
    ...
    if len(data["cookies"]) == 0:
        res = jsonify({"error": "no cookies found"})
        return make_response(res, 400)
    ...
    options.add_argument(
        "--user-agent=Mozilla/5.0 (X11; Linux x86_64; Ubuntu 22.04) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.0.0 Safari/537.36"
    )
    ...
    return jsonify(
        {"name": person.name, "profile_image": person.profile_image, "query": query}
    )
